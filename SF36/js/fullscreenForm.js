...
(function (window) {
	...
	FForm.prototype._deliverDatas = function () {
		...
		patientAge = this.formEl.querySelector('input[name="q39"]').value;
		patientID = this.formEl.querySelector('input[name="q40"]').value;
		if (!this.formEl.querySelector('input[name="q40"]').checkValidity()) {
			returnLable = false;
		}
		(function (obj) {
			var error;
			...
			case 'input':
				if (input.type === 'radio' || input.type === 'checkbox') {
					...
					if (input.value === '') {
						error = 'NOVAL';
						break;
					} else if (parseInt(input.value) > 200 || parseInt(input.value) < 1) {
						error = 'Age Error';
						break;
					}
				}
