...
(function (window) {
	...
	FForm.prototype._deliverDatas = function () {
		...
		patientAge = this.formEl.querySelector('input[name="q25"]').value;
		patientHeight = this.formEl.querySelector('input[name="q26"]').value;
		patientWeight = this.formEl.querySelector('input[name="q27"]').value;
		patientLung = this.formEl.querySelector('input[name="q28"]').value;
		patientID = this.formEl.querySelector('input[name="q29"]').value;
		if (!this.formEl.querySelector('input[name="q29"]').checkValidity()) {
			returnLable = false;
		}
		(function (obj) {
			var error;
			...
			if (fld.hasAttribute('data-input-trigger')) {
				...
				switch (input.tagName.toLowerCase()) {
					case 'select':
						...
						//		var element_name;
						//		elemrnt_name = inp.getAttribute('name');
						//		if (element_name.search('q3') != -1) {
						//			self.q3Label = true;
						//		}
						//		if ((element_name.search('q4') != -1 && self.q3Label == true) || element_name.search('q4') == -1) {
		...
		switch (input.tagName.toLowerCase()) {
			case 'input':
				if (input.type === 'radio' || input.type === 'checkbox') {
					...
					//console.log(checked);
					//console.log(fld.querySelectorAll('input[type="' + input.type + '"]').length);
					if (checked != name_num) {
						error = 'NOVAL';
					}
					...
					if (input.id === 'q25') {
						if (parseInt(input.value) > 200 || parseInt(input.value) < 1) {
							error = 'Age Error';
							break;
						}
					} else if (input.id === 'q26') {
						if (parseInt(input.value) > 275 || parseInt(input.value) < 1) {
							error = 'Height Error';
							break;
						}
					} else if (input.id === 'q27') {
						if (parseInt(input.value) > 1000 || parseInt(input.value) < 1) {
							error = 'Weight Error';
							break;
						}
					}
